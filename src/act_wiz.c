...
void do_vlist(CHAR_DATA *ch, char *argument)
{
    ...
    int j = 0;
    int vnum = 0;
    size_t buffer_len = 0; // Current length of the buffer
    size_t buffer_capacity = INITIAL_BUFFER_SIZE; // Current capacity of the buffer
    ...
    for (i = 0; i < MAX_ROW; i++)
    {
        buffer_len = 0; // Reset the buffer length for each row
        ...
        for (j = 0; j < COLUMNS; j++)
        {
            ...
            if (vnum < MAX_SHOW_VNUM)
            {
                ...
                char temp[100];
                snprintf(temp, sizeof(temp), "%3d %-8.8s  ", vnum, room ? area_name(room->area) : "-");
                size_t temp_len = strlen(temp);

                // Check if the buffer capacity is exceeded
                if (buffer_len + temp_len + 1 > buffer_capacity)
                {
                    // Resize the buffer to accommodate the new string
                    buffer_capacity = buffer_capacity * 2 + temp_len + 1;
                ...
                strncpy(buffer + buffer_len, temp, temp_len);
                buffer_len += temp_len;
                buffer[buffer_len] = '\0'; // Add the null-terminator
            }
        }
...
void do_election (CHAR_DATA *ch, char *argument)
{
	...
	run_election();
	// announce_election();
	send_to_char("\tGSuccess\tn: Election complete.\n\r", ch);
}
