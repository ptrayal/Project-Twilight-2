...
long fread_long( FILE *fp )
{
    ...
    if (!isdigit(c))
    {
        log_string(LOG_BUG, "Fread_long: bad format.");
        exit(1);
    }
...
void name_replacer(char *format, char *first_name, char *last_name, char *out)
{
    ...
    while ( *str != '\0' )
    {
        ...
        {
        default:
            log_string(LOG_BUG, Format("Rand_name: bad code %d.", *str ));
            i = " Dave ";
            break;
        case 'f':
            i = first_name;
