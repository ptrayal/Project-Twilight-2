...
#define CHK_ISPC        (11)
#define CHK_ISNPC       (12)
#define CHK_ISFRENZY    (13)
#define CHK_ISDRUNK     (14)
#define CHK_ISNATURAL   (15)
#define CHK_ISADMIN     (16)
...
#define CHK_SEX         (42)
#define CHK_MONEY       (43)
#define CHK_OBJVAL0     (44)
#define CHK_OBJVAL1     (45)
#define CHK_OBJVAL2     (46)
#define CHK_OBJVAL3     (47)
#define CHK_OBJVAL4     (48)
#define CHK_GRPSIZE     (49)
#define CHK_ISHIGH      (50)
#define CHK_ISTRIPPING  (51)
#define CHK_ISAFRAID    (52)
#define CHK_ISHUNTED    (53)
#define CHK_ISWANTED    (54)
...
const char * fn_keyword[] =
{
    ...
    "isfrenzy",		/* if isfrenzy $n 	- is $n frenzied */
    "isdrunk",		/* if isdrunk $n 	- is $n drunk */
    "isnatural",	/* if isnatural $n 	- is $n natural */
    "isadmin",		/* if isadmin $n	- is $n on staff */
    ...
    "org",			/* if org $n mage	- is $n a member of org 'mage' */
    ...
    "ishigh",		/* if ishigh $n		- is $n high */
    "istripping",	/* if istripping $n	- is $n tripping */
    "isafraid",		/* if isafraid $n	- is $n afraid */
    "ishunted",		/* if ishunted $n	- is $n hunted */
    "iswanted",		/* if iswanted $d	- is $n wanted */
 ...
 *----------------------------------------------------------------------
 */
int cmd_eval( sh_int vnum, char *line, int check, CHAR_DATA *mob, CHAR_DATA *ch,  const void *arg1, const void *arg2, CHAR_DATA *rch )
{
...
switch( check )
{
    ...
    return( lval_char != NULL && IS_NPC( lval_char ) );
    case CHK_ISFRENZY:
    return( lval_char != NULL && NEAR_FRENZY( lval_char ) );
    case CHK_ISDRUNK:
    return( lval_char != NULL && IS_DRUNK( lval_char ) );
    case CHK_ISHIGH:
    return( lval_char != NULL && IS_HIGH( lval_char ) );
    case CHK_ISTRIPPING:
    return( lval_char != NULL && IS_TRIPPING( lval_char ) );
    case CHK_ISAFRAID:
    return( lval_char != NULL && IS_FEARFUL( lval_char ) );
    case CHK_ISHUNTED:
    return( lval_char != NULL && IS_HUNTED( lval_char ) );
    case CHK_ISWANTED:
    return( lval_char != NULL && IS_WANTED( lval_char ) );
    case CHK_ISNATURAL:
    return( lval_char != NULL && IS_NATURAL( lval_char ) );
    case CHK_ISADMIN:
    return( lval_char != NULL && IS_ADMIN( lval_char ) );
    ...
    case CHK_ISACTIVE:
    return( lval_char != NULL && lval_char->position > P_SLEEP );
    case CHK_ISDELAY:
...
case CHK_ORG:
return( lval_char != NULL
  && (mem_lookup(org_lookup(buf), lval_char->name) != NULL));
case CHK_OBJTYPE:
...
case CHK_HPCNT:
if ( lval_char != NULL )
  lval = ((lval_char->health+lval_char->agghealth-7) * 100)
/7;
break;
case CHK_ROOM:
if ( lval_char != NULL && lval_char->in_room != NULL )
{
    ...
    break;
	case CHK_MONEY:  /* Money is converted to cents... */
    if ( lval_char != NULL )
    {
        lval = lval_char->cents + (lval_char->dollars * 100);
    }
    break;
    case CHK_OBJVAL0:
    if ( lval_obj != NULL )
 ...
 *  Lines beginning with 'mob' are passed to mprog_interpret() to handle
	...
	CHAR_DATA *mob, CHAR_DATA *ch, const void *arg1, const void *arg2 )
{
    ...
    char *code, *line;
    char buf[MSL]={'\0'};
    char control[MAX_INPUT_LENGTH]={'\0'};
    char data[MSL]={'\0'};
	...
	*b = *c = *d = '\0';
	...
	if ( buf[0] == '*' ) /* Comment */
        ...
        else if ( (!level || cond[level] == TRUE) && !IS_NULLSTR(buf))
        {
        	state[level] = IN_BLOCK;
...
void mp_antiobj_trigger( CHAR_DATA *mob, CHAR_DATA *ch, OBJ_DATA *obj )
{
