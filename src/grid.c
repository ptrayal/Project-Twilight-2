/* grid.c. Where grids are created, displayed and destroyed. Davion. MudBytes.net */

#if defined(Macintosh)
#include <types.h>
#else
#include <sys/types.h>
#include <sys/time.h>
#endif

#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdarg.h>

#include "twilight.h"
#include "grid.h"

//Creation/Destruction
GRID_DATA *create_grid(int width)
{
    GRID_DATA *new_grid;

    new_grid = calloc(1, sizeof(*new_grid));
    if (!new_grid)
    {
        log_string(LOG_ERR, "create_grid: out of memory");
        return NULL;
    }
    new_grid->border_corner   = '+';
    new_grid->border_left     = '|';
    new_grid->border_right    = '|';
    new_grid->border_internal = '+';
    new_grid->border_top      = '_';
    new_grid->border_bottom   = '-';
    new_grid->width           = width;
    return new_grid;
}

GRID_DATA *destroy_grid( GRID_DATA *grid )
{
    GRID_ROW *row, *row_next;

    for (row = grid->first_row; row; row = row_next)
    {
        row_next = row->next;
        destroy_row(row);   /* destroy_row now frees its own cells */
    }
    free(grid);
    return NULL;
}

GRID_ROW *create_row(GRID_DATA *grid)
{
    GRID_ROW *new_row;

    new_row = calloc(1, sizeof(*new_row));
    if (!new_row)
    {
        log_string(LOG_ERR, "create_row: out of memory");
        return NULL;
    }
    new_row->grid = grid;
    if (grid)
        grid_add_row(grid, new_row);
    new_row->padding_top    = 0;
    new_row->padding_bottom = 0;
    new_row->padding_left   = 1;
    new_row->padding_right  = 1;
    return new_row;
}

GRID_ROW *destroy_row( GRID_ROW *row )
{
    GRID_CELL *cell, *cell_next;

    if (row->grid)
        grid_remove_row(row);
    for (cell = row->first_cell; cell; cell = cell_next)
    {
        cell_next = cell->next;
        destroy_cell(cell);
    }
    free(row);
    return NULL;
}

GRID_CELL *create_cell(GRID_ROW *row, int width)
{
    GRID_CELL *new_cell;

    new_cell = calloc(1, sizeof(*new_cell));
    if (!new_cell)
    {
        log_string(LOG_ERR, "create_cell: out of memory");
        return NULL;
    }
    new_cell->contents[0] = '\0';
    new_cell->width       = width;
    new_cell->row         = row;
    if (row)
        row_add_cell(row, new_cell);
    return new_cell;
}

GRID_CELL *destroy_cell(GRID_CELL *cell)
{
    if (cell->row)
        row_remove_cell(cell);
    free(cell);
    return NULL;
}

//List management
void grid_add_row(GRID_DATA *grid, GRID_ROW *row)
{
    if (!grid->last_row)
    {
        grid->first_row = row;
        grid->last_row  = row;
        row->grid       = grid;
        return;
    }
    row->prev            = grid->last_row;
    grid->last_row->next = row;
    grid->last_row       = row;
    row->grid            = grid;
}

void grid_remove_row( GRID_ROW *row )
{
    GRID_DATA *grid = row->grid;
    if (!grid)
        return;
    if (row == grid->first_row)
        grid->first_row = grid->first_row->next;
    if (row == grid->last_row)
        grid->last_row = row->prev;
    if (row->prev)
        row->prev->next = row->next;
    if (row->next)
        row->next->prev = row->prev;
    row->grid = NULL;
    row->next = NULL;
    row->prev = NULL;
}

void row_add_cell( GRID_ROW *row, GRID_CELL *cell)
{
    if (row->curr_width + cell->width > row->grid->width)
        log_string(LOG_ERR, "Warning: Added Cell Width Overflows Grid");

    if (!row->last_cell)
    {
        row->first_cell  = cell;
        row->last_cell   = cell;
        cell->row        = row;
        row->curr_width += cell->width;    /* Bug fix: first cell must update curr_width */
        row->columns++;
        return;
    }
    cell->prev           = row->last_cell;
    row->last_cell->next = cell;
    row->last_cell       = cell;
    cell->row            = row;
    row->curr_width     += cell->width;
    row->columns++;
}

void row_remove_cell( GRID_CELL *cell)
{
    GRID_ROW *row = cell->row;
    if (!row)
        return;
    if (cell == row->first_cell)
        row->first_cell = row->first_cell->next;
    if (cell == row->last_cell)
        row->last_cell = cell->prev;
    if (cell->prev)
        cell->prev->next = cell->next;
    if (cell->next)
        cell->next->prev = cell->prev;
    row->columns--;
    row->curr_width -= cell->width;
    cell->row  = NULL;
    cell->next = NULL;
    cell->prev = NULL;
}

//Meat
//IF You already have a cell use this to set the contents. If it has contents already, they are overwritten.
void cell_set_contents (GRID_CELL *cell, char *fmt, ...)
{
    char buf[MSL];
    va_list args;
    va_start(args, fmt);
    vsnprintf(buf, MSL, fmt, args);     /* Bug fix: was vsprintf, no length limit */
    va_end(args);
    strncpy(cell->contents, buf, MSL - 1);
    cell->contents[MSL - 1] = '\0';
    cell_set_linecount(cell);
}

//If you already have a cell use this to append to the cells content.
void cell_append_contents (GRID_CELL *cell, char *fmt, ...)
{
    char buf[MSL];
    va_list args;
    va_start(args, fmt);
    vsnprintf(buf, MSL, fmt, args);
    va_end(args);

    if (strlen(cell->contents) + strlen(buf) < MSL - 1)
    {
        strncat(cell->contents, buf, MSL - strlen(cell->contents) - 1);
    }
    else
    {
        log_string(LOG_ERR, "cell_append_contents: content truncated, MSL exceeded");
    }
    cell_set_linecount(cell);
}

//IF you don't already have a cell, but you have a row, use this to create a cell with the contents of fmt.
GRID_CELL *row_append_cell (GRID_ROW *row, int width, char *fmt, ...)
{
    char buf[MSL];
    GRID_CELL *cell;
    va_list args;
    va_start(args, fmt);
    vsnprintf(buf, MSL, fmt, args);     /* Bug fix: was vsprintf, no length limit */
    va_end(args);

    cell = create_cell(row, width);
    cell_set_contents(cell, "%s", buf); /* Bug fix: was cell_set_contents(cell, buf) — format string injection */
    return cell;
}

//Low level line counter
void cell_set_linecount(GRID_CELL *cell)
{
    int count = 1;
    char *pos = cell->contents;

    /* Bug fix: original used while(*pos++) which incremented before the body,
       causing the newline check to always look at the wrong character. */
    while (*pos)
    {
        if (*pos == '\n')
            count++;
        pos++;
    }

    cell->lines = count;

    if (cell->row->max_height < count)
        cell->row->max_height = count;
}

//Counts colour codes to display offsets properly
int count_colour ( char *str )
{
    char c;
    int count = 0;
    while ((c = *str++) != '\0')
    {
        if (c == '\t')
        {
            count++;
            str++;      /* skip the code letter */
        }
        if (c == '^')
        {
            count++;
            str++;      /* Bug fix: was missing — ^ codes are also 2-char sequences */
        }
        if (c == '{' || c == '@')
        {
            count++;
            str++;      /* skip the code letter */
        }
    }
    return count * 2;
}

//Displaying of the Grid
void row_to_char( GRID_ROW *row, CHAR_DATA *ch )
{
    GRID_CELL *cell;
    char ***ptrs;
    char *tok;
    char buf[MSL], pad_buf[MSL], content_copy[MSL], line_buf[MSL * 4];
    int i = 0, n = 0, llen;
    int filler       = UMAX(0, row->grid->width - row->curr_width - 1); /* Bug fix: was UMIN, always gave 0 */
    int actual_height = row->max_height + (row->padding_top + row->padding_bottom);

    ptrs = calloc(row->columns, sizeof(*ptrs));
    if (!ptrs)
    {
        log_string(LOG_ERR, "row_to_char: out of memory");
        return;
    }

    /* Explode columns into individual lines. */
    for (i = 0, cell = row->first_cell; cell; cell = cell->next, ++i)
    {
        int colour_offset = 0;
        int actual_width  = cell->width - row->padding_right - row->padding_left - 1;

        sprintf(pad_buf, "%*s%-*.*s%*s",
            row->padding_left, " ",
            actual_width, actual_width, " ",
            row->padding_right, " ");

        ptrs[i] = calloc(actual_height, sizeof(*ptrs[i]));
        if (!ptrs[i])
        {
            int j, k;
            log_string(LOG_ERR, "row_to_char: out of memory for column");
            for (j = 0; j < i; j++)
            {
                for (k = 0; k < actual_height; k++)
                    free(ptrs[j][k]);
                free(ptrs[j]);
            }
            free(ptrs);
            return;
        }

        /* Bug fix: strtok modifies cell->contents in place, breaking any subsequent
           display or linecount. Work on a local copy instead. */
        strncpy(content_copy, cell->contents, MSL - 1);
        content_copy[MSL - 1] = '\0';
        tok = strtok(content_copy, "\n");

        /* Append padding to top */
        for (n = 0; n < row->padding_top; ++n)
            ptrs[i][n] = strdup(pad_buf);

        while (tok)
        {
            colour_offset = count_colour(tok);
            sprintf(buf, "%*s%-*.*s%*s",
                row->padding_left, " ",
                actual_width + colour_offset, actual_width + colour_offset, tok,
                row->padding_right, " ");
            ptrs[i][n] = strdup(buf);
            tok = strtok(NULL, "\n");
            ++n;
        }

        /* Fill remaining lines (bottom padding + any short columns) */
        for (; n < actual_height; ++n)
            ptrs[i][n] = strdup(pad_buf);
    }

    /* Performance fix: build each full output line in a buffer; one send_to_char per line
       instead of multiple Format+send_to_char calls per border character and cell. */
    for (n = 0; n < actual_height; ++n)
    {
        llen = 0;
        for (i = 0; i < row->columns; ++i)
        {
            int slen;
            line_buf[llen++] = (i == 0) ? row->grid->border_left : row->grid->border_internal;
            slen = strlen(ptrs[i][n]);
            if (llen + slen < (int)(sizeof(line_buf)) - 8)
            {
                memcpy(line_buf + llen, ptrs[i][n], slen);
                llen += slen;
            }
            free(ptrs[i][n]);
        }

        if (filler > 0 && llen + filler < (int)(sizeof(line_buf)) - 4)
        {
            memset(line_buf + llen, ' ', filler);
            llen += filler;
        }
        line_buf[llen++] = row->grid->border_right;
        line_buf[llen++] = '\r';
        line_buf[llen++] = '\n';
        line_buf[llen]   = '\0';
        send_to_char(line_buf, ch);
    }

    for (i = 0; i < row->columns; ++i)
        free(ptrs[i]);
    free(ptrs);
}

//Display the whole grid, row by row, and destroy if necessary. Should only not destroy if you intend to display it multiple times in a single function.
void grid_to_char(GRID_DATA *grid, CHAR_DATA *ch, bool destroy)
{
    GRID_ROW *row;
    char border[MSL];
    int i, len;

    /* Performance fix: build the border string once and send in one call
       instead of one send_to_char per character. */
    len = 0;
    border[len++] = grid->border_corner;
    for (i = 0; i < grid->width - 1 && len < MSL - 3; ++i)
        border[len++] = grid->border_top;
    border[len++] = grid->border_corner;
    border[len++] = '\r';
    border[len++] = '\n';
    border[len]   = '\0';
    send_to_char(border, ch);

    for (row = grid->first_row; row; row = row->next)
    {
        row_to_char(row, ch);

        len = 0;
        border[len++] = grid->border_corner;
        for (i = 0; i < grid->width - 1 && len < MSL - 3; ++i)
            border[len++] = grid->border_bottom;
        border[len++] = grid->border_corner;
        border[len++] = '\r';
        border[len++] = '\n';
        border[len]   = '\0';
        send_to_char(border, ch);
    }

    if (destroy)
        destroy_grid(grid);
}
