...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    else
    {
        ...
        {
        case P_DEAD:
            strncat( buf, " is DEAD!!", sizeof(buf) - strlen(buf) - 1 );
            break;
        case P_MORT:
            strncat( buf, " is mortally wounded.", sizeof(buf) - strlen(buf) - 1 );
            break;
        case P_INCAP:
            strncat( buf, " is incapacitated.", sizeof(buf) - strlen(buf) - 1 );
            break;
        case P_STUN:
            strncat( buf, " is lying here stunned.", sizeof(buf) - strlen(buf) - 1 );
            break;
        case P_SLEEP:
            if (victim->on != NULL)
            {
...
void do_nature (CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("\tRWarning:\tn  That archetype is not recognized.\n\r", ch);
        return;
    }
    ...
    send_to_char( "\tGSuccess\tn: Your character's Nature has been set.\n\r", ch );
}
...
void do_demeanor (CHAR_DATA *ch, char *argument)
{
    ...
    send_to_char( "\tGSuccess\tn: Your character's Demeanor has been set.\n\r", ch );
}
...
void do_surname(CHAR_DATA *ch, char *argument)
{
    ...
    send_to_char("\tGSuccess:\tn  Your lastname has been set.\n\r", ch);
    PURGE_DATA(ch->surname);
