...
void do_asave( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char( "\tRWARNING:\tn That area does not exist.\n\r", ch );
		return;
	}
	...
	if ( is_number( arg1 ) )
	{
		...
		{
			send_to_char( "\tRWARNING:\tn You are not a builder for this area.\n\r", ch );
			return;
		}
		save_area_list();
		save_area( pArea );
	...
	if ( !str_cmp( "world", arg1 ) )
	{
		...
		send_to_char( "\tGSuccess\tn: The world has been saved.\n\r", ch );
...
void do_help_unformatted (CHAR_DATA *ch, char *argument)
{
	...
	send_to_char("\tYHelps with unformatted still\tn:\n\r", ch);
	...
	send_to_char("\tGSuccess\tn: All help files still unformatted are now listed.\n\r", ch);
	return;
}
