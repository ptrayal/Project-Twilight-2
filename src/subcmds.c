...
bool CAN_BE(CHAR_DATA *ch, int cmd)
{
	...
	if(IS_VAMPIRE(ch) && IS_SET(job_table[cmd].available, VA))
		return TRUE;
...
int home_list(CHAR_DATA *ch, char *argument, int type, ORG_DATA *org)
{
	...
	if(type)
	{
		send_to_char("\tRWARNING:\tn At this time, it is not possible to list the rooms owned by an organization.\n\r", ch);
		return FALSE;
	}

	if(ch->home == 0)
	{
		send_to_char("\tRWARNING:\tn You do not own a home.\n\r", ch);
		return FALSE;
	}
