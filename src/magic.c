...
void do_protean4 (CHAR_DATA *ch, char *argument)
{
	...
	if(ch->race == race_lookup("vampire") && ch->disc[DISC_PROTEAN] >= 4)
	{
		if(IS_NULLSTR(arg))
		{
			send_to_char("\tRWARNING:\tn Change into what?\n\r", ch);
			return;
		}
		...
		send_to_char("\tRWARNING:\tn There is no such form.\n\r", ch);
	}
	else
	{
		send_to_char("\tRWARNING:\tn You have not mastered this power of Protean.\n\r", ch);
	}
}
...
void do_dominate1 (CHAR_DATA *ch, char *string)
{
	...
	{
			send_to_char("\tRWarning: You do not know Compel\tn.\n\r", ch);
		return;
	}
...
void do_dominate2 (CHAR_DATA *ch, char *string)
{
	...
	{
		send_to_char("\tRWarning: You do not know Mesmerize\tn.\n\r", ch);
		return;
	}
	...
	if (IS_NULLSTR(arg))
	{
		send_to_char("Whom are you trying to Mesmerize?\n\r", ch);
		return;
	}
	...
	difficulty = victim->willpower;
	power_stat = get_curr_stat(ch, STAT_MAN);
	power_ability = ch->ability[LEADERSHIP].value;
	success = dice_rolls(ch, power_stat + power_ability, difficulty);
	...
	if ((ch->race = race_lookup("vampire")) && (ch->disc[DISC_DOMINATE] >= 1)) 
	{
		if (ch->trust < victim->trust || (ch->gen > victim->gen && !IS_NATURAL(victim)))
		{
			send_to_char(Format("%s is beyond your ability to influence.\n\r", victim->name), ch);
			return;
		}
		if (success > 0)
		{
...
void do_obfuscate1 (CHAR_DATA *ch, char *argument)
{
	...
	if( (!IS_VAMPIRE(ch) && !IS_WEREWOLF(ch) )
		|| (IS_VAMPIRE(ch) && ch->disc[DISC_OBFUSCATE] < 1)
		|| (IS_WEREWOLF(ch) && !IS_SET(ch->powers[1], F)) )
	{
		send_to_char("\tRWarning: You do not know Cloak of Shadows\tn.\n\r", ch);
		return;
	}
...
void do_obfuscate2 (CHAR_DATA *ch, char *argument)
{
	...
	if( ( !IS_VAMPIRE(ch) && !IS_WEREWOLF(ch) )
		|| ( IS_VAMPIRE(ch) && ch->disc[DISC_OBFUSCATE] < 2) )
	{
		send_to_char("\tRWarning: You do not know Unseen Presence\tn.\n\r", ch);
		return;
	}
...
void do_obfuscate3 (CHAR_DATA *ch, char *string)
{
	...
	if( ( !IS_VAMPIRE(ch) && !IS_WEREWOLF(ch) )
		|| (IS_VAMPIRE(ch) && ch->disc[DISC_OBFUSCATE] < 3)
		|| (IS_WEREWOLF(ch) && !IS_SET(ch->powers[3], C)) )
	{
		send_to_char("\tRWarning: You do not know Mask of a Thousand Faces\tn.\n\r", ch);
		return;
	}
	...
	if(IS_NULLSTR(string))
	{
		send_to_char("That does not change your face.\n\r", ch);
		return;
	}
	...
	{
		send_to_char("You cannot change your looks with others around.\n\r", ch);
	}
}
...
void do_obfuscate4 (CHAR_DATA *ch, char *argument)
{
	...
	if( (IS_VAMPIRE(ch) && ch->disc[DISC_OBFUSCATE] < 4) )
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
void do_serpentis1 (CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("\tRWARNING:\tn You have not mastered this power of Serpentis.\n\r", ch);
		return;
	}
...
void do_serpentis3 (CHAR_DATA *ch, char *argument)
{
	...
	if(ch->willpower <= 1)
	{
		send_to_char("\tRWARNING:\tn You don't have the willpower to do that.\n\r", ch);
		return;
	}
...
void do_serpentis4 (CHAR_DATA *ch, char *argument)
{
	...
	if(IS_VAMPIRE(ch) && ch->disc[DISC_SERPENTIS] >= 4)
	{
		if (!has_enough_power(ch))
		{
			return;
