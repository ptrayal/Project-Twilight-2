...
// --- Safer helper replacements for macros ---
void WriteStringSafe(FILE *fp, const char *key, const char *value, bool newline, bool append_delim)
{
    if (fp && key && value && *key && *value)
        fprintf(fp, "%s %s%s%s", key, value, append_delim ? DELIMITER : "", newline ? "\n" : "");
}

void WriteNumberSafe(FILE *fp, const char *key, int value)
{
    if (fp && key && *key)
        fprintf(fp, "%s %d\n", key, value);
}

void WriteLongSafe(FILE *fp, const char *key, long value)
{
    if (fp && key && *key)
        fprintf(fp, "%s %ld\n", key, value);
}
...
void sanitize_affected_flags(CHAR_DATA *ch)
{
    if (IS_SET(ch->affected_by2, AFF2_EARTHMELD))
        REMOVE_BIT(ch->affected_by2, AFF2_EARTHMELD);
}

// --- Phase 2: Header / Identity ---
void write_header(FILE *fp, CHAR_DATA *ch, time_t now)
{
    time_t current_time = time(NULL); // Get the current epoch time

    fprintf(fp, "#%s\n", IS_NPC(ch) ? "MOB" : "PLAYER");
    WriteNumberSafe(fp, "Ver", ch->version);
    WriteStringSafe(fp, "Name", ch->name, true, true);
    WriteStringSafe(fp, "SName", ch->surname, true, true);
    WriteStringSafe(fp, "AName", ch->alt_name, true, true);
    WriteStringSafe(fp, "Email", ch->email_addr, true, true);
    WriteLongSafe(fp, "Econfnum", ch->email_lock );

    WriteLongSafe(fp, "Id", ch->id);
    WriteLongSafe(fp, "LogO", current_time);
    WriteLongSafe(fp, "LastV", ch->pcdata->last_vote);

    if ( IS_NPC(ch) )
    {
        WriteNumberSafe(fp, "Vnum", ch->pIndexData->vnum);
...
void write_basic_identity(FILE *fp, CHAR_DATA *ch)
{
    WriteStringSafe(fp, "ShD", ch->short_descr, true, true);
    WriteStringSafe(fp, "LnD", ch->long_descr, true, true);
    WriteStringSafe(fp, "Desc", ch->description, true, true);
    WriteStringSafe(fp, "SDesc", ch->switch_desc, true, true);
    WriteNumberSafe(fp, "Race", ch->race);
    fprintf(fp, "AusBr %d %d\n", ch->auspice, ch->breed);
    WriteNumberSafe(fp, "Gen", ch->gen);
    WriteNumberSafe(fp, "Shape", ch->shape);
    WriteNumberSafe(fp, "Warrants", ch->warrants);
    fprintf(fp, "Hunted %d\n", ch->hunter_vis);
    WriteStringSafe(fp, "Cln", clan_table[ch->clan].name, true, true);
    WriteStringSafe(fp, "Material", ch->material, true, true);
    fprintf(fp, "CPow %d %d %d\n", ch->clan_powers[0], ch->clan_powers[1], ch->clan_powers[2]);
    WriteNumberSafe(fp, "Sex", ch->sex);
    WriteNumberSafe(fp, "Not", ch->pcdata->last_note);
    WriteNumberSafe(fp, "Scro", ch->lines);
    WriteStringSafe(fp, "Marry", ch->married, true, true);
    WriteStringSafe(fp, "Ghld", ch->ghouled_by, true, true);
    WriteStringSafe(fp, "Nature", ch->nature, true, true );
    WriteStringSafe(fp, "Demnor", ch->demeanor, true, true );
    WriteStringSafe(fp, "Pack", ch->pack, true, true );
    ...
    // WriteStringSafe(fp, "Sire", sire_table[ch->sire].name, true, true);
}

void write_stats(FILE *fp, CHAR_DATA *ch, time_t now)
{
    WriteNumberSafe(fp, "Playd", ch->played + (int)(now - ch->logon));
	WriteNumberSafe(fp, "Age", ch->char_age );
    int room_vnum = (ch->in_room == get_room_index(ROOM_VNUM_LIMBO) && ch->was_in_room)
                    ? ch->was_in_room->vnum : ch->in_room ? ch->in_room->vnum : 10000;
    WriteNumberSafe(fp, "Room", room_vnum);
    WriteNumberSafe(fp, "Listen", ch->listening ? ch->listening->vnum : 0);
    WriteStringSafe(fp, "Prompt", ch->prompt, true, true);
    ...
    WriteNumberSafe(fp, "Pos", ch->position == P_FIGHT ? P_STAND : ch->position);

    WriteNumberSafe(fp, "Save", ch->saving_throw );

    fprintf(fp, "Attr");
    for (int i = 0; i < 9; i++) fprintf(fp, " %d", ch->perm_stat[i]);
    fprintf(fp, "\nAMod");
    for (int i = 0; i < 9; i++) fprintf(fp, " %d", ch->mod_stat[i]);
    fprintf(fp, "\n");

	WriteNumberSafe(fp, "Herdt", ch->herd_timer );
	WriteNumberSafe(fp, "BgTime", ch->bg_timer );
	WriteNumberSafe(fp, "BgCount", ch->bg_count );
...
void write_powers_backgrounds(FILE *fp, CHAR_DATA *ch)
{
    fprintf(fp, "Dsc ");
    for (int i = 0; i < MAX_DISC; i++) fprintf(fp, "%d ", ch->disc[i]);
    fprintf(fp, "\nPow ");
    for (int i = 0; i < MAX_POWERS; i++) fprintf(fp, "%s ", print_flags(ch->powers[i]));
    fprintf(fp, "\nStatus ");
    for (int i = MAX_BACKGROUND; i < MAX_BG; i++) fprintf(fp, "%d ", ch->backgrounds[i]);
    fprintf(fp, "\nInfl ");
    for (int i = 0; i < MAX_INFL; i++) fprintf(fp, "%d ", ch->influences[i]);
    fprintf(fp, "\nBack ");
    for (int i = 0; i < MAX_BACKGROUND; i++) fprintf(fp, "%d ", ch->backgrounds[i]);
    fprintf(fp, "\n");
}

void write_financial_data(FILE *fp, CHAR_DATA *ch)
{
    WriteNumberSafe(fp, "Dollars", ch->dollars > 0 ? ch->dollars : 0);
    WriteNumberSafe(fp, "Cents", ch->cents > 0 ? ch->cents : 0);
    if (ch->bank_account > 0)
        WriteLongSafe(fp, "Bank", ch->bank_account);
    else
        WriteNumberSafe(fp, "Bank", 0);
    WriteNumberSafe(fp, "Exp", ch->exp);
    WriteNumberSafe(fp, "OocExp", ch->oocxp);
    fprintf(fp, "Oocxp %d %d\n", ch->ooc_xp_time, ch->ooc_xp_count );
}

void write_traits_and_stocks(FILE *fp, CHAR_DATA *ch)
{
    for (STOCKS *stock = ch->stocks; stock; stock = stock->next)
    {
        if (stock->cost > 0)
            fprintf(fp, "STOCK %s~ %d\n", stock->ticker, stock->cost);
    }
    for (TRAIT_DATA *trait = ch->traits; trait; trait = trait->next)
    {
        fprintf(fp, "Trait %d %d %s~ %s~\n", trait->type, trait->value, trait->qualifier, trait->detail);
    }
}

void write_player_data(FILE *fp, CHAR_DATA *ch)
{
    int sn = 0;

    WriteStringSafe(fp, "Pass", ch->pcdata->pwd, true, true);
    WriteStringSafe(fp, "Bin", ch->pcdata->bamfin, true, true);
    WriteStringSafe(fp, "Bout", ch->pcdata->bamfout, true, true);
    WriteStringSafe(fp, "Titl", ch->pcdata->title, true, true);
    WriteStringSafe(fp, "RPTitle", ch->pcdata->rpok_string, true, true);
    fprintf(fp, "Colours %s~ %s~\n", ch->colours[0], ch->colours[1]);
    WriteNumberSafe(fp, "Pnts", ch->pcdata->points);
    WriteNumberSafe(fp, "Gift", ch->xpgift);
    WriteNumberSafe(fp, "TSex", ch->pcdata->true_sex);

    if (ch->act != 0)
        WriteStringSafe(fp, "Act", print_flags(ch->act), true, true);
        // WriteToFile( fp, true, "Act", print_flags(ch->act) );
    if (ch->act2 != 0)
        WriteStringSafe(fp, "Act2", print_flags(ch->act2), true, true);
        // WriteToFile( fp, true, "Act2", print_flags(ch->act2) );
    if (ch->plr_flags != 0)
        WriteStringSafe(fp, "Pflgs", print_flags(ch->plr_flags), true, true);
        // WriteToFile( fp, true, "Pflgs", print_flags(ch->plr_flags) );
    if (ch->affected_by != 0)
        WriteStringSafe(fp, "AfBy", print_flags(ch->affected_by), true, true);
        // WriteToFile( fp, true, "AfBy", print_flags(ch->affected_by) );
    if (ch->affected_by2 != 0)
        WriteStringSafe(fp, "AfBy2", print_flags(ch->affected_by2), true, true);
        // WriteToFile( fp, true, "AfBy2", print_flags(ch->affected_by2) );
    if (ch->parts != race_table[ch->race].parts)
        WriteStringSafe(fp, "Part", print_flags(ch->parts), true, true);
        // WriteToFile( fp, true, "Part", print_flags(ch->parts) );
    if (ch->form != race_table[ch->race].form)
        WriteStringSafe(fp, "Form", print_flags(ch->form), true, true);
        // WriteToFile( fp, true, "Form", print_flags(ch->form) );

    WriteStringSafe(fp, "Comm", print_flags(ch->comm), true, true);

    fprintf(fp, "Condit");
    for (int i = 0; i < 11; i++) fprintf(fp, " %d", ch->condition[i]);
    fprintf(fp, "\n");
    
    WriteNumberSafe(fp, "Home", ch->home);
    if (ch->home > 0)
    {
        fprintf(fp, "MyRms ");
        for (int i = 0; i < ch->home; i++)
        {
            if (ch->rooms[i])
                fprintf(fp, "%d ", ch->rooms[i]->vnum);
        }
        fprintf(fp, "\n");
    }
    ...
    {
        if (!ch->pcdata->alias[i] || !ch->pcdata->alias_sub[i]) break;
        fprintf(fp, "Alias %s %s~\n", ch->pcdata->alias[i], ch->pcdata->alias_sub[i]);
    }
    
    for (int sn = 0; sn < MAX_SKILL; sn++)
    {
        if (skill_table[sn].name && ch->learned[sn] > 0)
...
void write_immortal_info(FILE *fp, CHAR_DATA *ch)
{
    WriteNumberSafe(fp, "Tru", ch->trust);
    WriteNumberSafe(fp, "Sec", ch->pcdata->security);
    WriteStringSafe(fp, "Wizn", print_flags(ch->wiznet), true, true);
    WriteNumberSafe(fp, "Invi", ch->invis_level);
    WriteNumberSafe(fp, "Inco", ch->incog_level);


}

void write_end_marker(FILE *fp, CHAR_DATA *ch)
{
    WriteStringSafe(fp, "Reject", ch->pcdata->ignore_reject, true, false);
    WriteStringSafe(fp, "Block", ch->pcdata->block_join, true, false);
    WriteStringSafe(fp, "Ign", ch->ignore, true, false);
    fprintf(fp, "End\n\n");
}
...
void fwrite_char(CHAR_DATA *ch, FILE *fp)
{
    ...
    sanitize_affected_flags(ch);
    write_header(fp, ch, now);
    write_basic_identity(fp, ch);
    write_immortal_info(fp, ch);
    write_stats(fp, ch, now);
    write_powers_backgrounds(fp, ch);
    write_financial_data(fp, ch);
    write_traits_and_stocks(fp, ch);
    if (!IS_NPC(ch)) write_player_data(fp, ch);
    write_affects(fp, ch);
    write_end_marker(fp, ch);
}
